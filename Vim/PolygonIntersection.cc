...
int main() {
    ...
    for (ll cs = 1; cin >> n && n >= 3; ++cs) {
        ...
        one = VP(one.rbegin(), one.rend());
        two = VP(two.rbegin(), two.rend());

        VP intersection = ConvexClipPolygon(one, two);

        sort(intersection.begin(), intersection.end(), lex_cmp_xy);

        VP::iterator it = unique(intersection.begin(), intersection.end());
        intersection.resize(distance(intersection.begin(), it));
