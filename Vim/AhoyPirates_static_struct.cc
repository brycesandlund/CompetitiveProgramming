...
int main() {
    ...
    for (ll cs = 1; cs <= nCases; ++cs) {
        ...
        for (ll i = 0; i < Q; ++i) {
            ...
            if (type == "F") {
                tree.rangeUpdate(1, 0, length-1, a, b, false, true);
            }
            else if (type == "E") {
                tree.rangeUpdate(1, 0, length-1, a, b, true, true);
            }
            else if (type == "I") {
                tree.rangeUpdate(1, 0, length-1, a, b, true, false);
            }
            else {
                SegmentNode result = tree.query(1, 0, length-1, a, b);
