...
void merge(vi &UF, ll i, ll j) { UF[i] = find(UF, j); }

int main() {
    ll limit = 1000000000000LL;
    for (ll i = limit - 1000000; i < limit; ++i) {
        vi cur = prime_factor_smart(i);
        cerr << i << endl;
    }
    ...
    for (ll cs = 1; cs <= C; ++cs) {
        ...
        vb sieve((ll)(sqrt(B)+EP)+1, true);
        for (ll i = 2; i < (ll)sieve.size(); ++i) {
            if (sieve[i]) {
                for (ll j = i*2; j < (ll)sieve.size(); j += i) {
                    sieve[j] = false;
                }
            }
        }

        for (ll i = P; i < (ll)sieve.size(); ++i) {
            if (sieve[i]) {
                for (ll j = ((A+i-1)/i) * (i+1); j <= B; j += i) {
                    merge(UF, j-A, j-i-A);
                }
            }
        }
