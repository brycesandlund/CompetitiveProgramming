...
ll getIdx(ll num, map<ll, ll> &pToIdx) {
    if (!pToIdx.count(num)) {
        ll size = pToIdx.size();
        pToIdx[num] = size;
    }
...
int main() {
    ll num = 234287384728LL;
    vi factors = prime_factor_smart(num);
    ...
    for (ll cs = 1; cs <= C; ++cs) {
        ...
        for (ll i = A; i <= B; ++i) {
            ...
            for (ll j = curFacts.size()-1; j >= 0 && curFacts[j] >= P; --j) {
                factors[i].push_back(getIdx(curFacts[j], pToIdx));
            }
        }
