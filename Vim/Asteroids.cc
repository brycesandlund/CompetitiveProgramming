...
double area(double time, VP p1, VP p2, v) {
    for (ll i = 0; i < p1.size(); ++i) {
        p1[i] = p1[i] + v*time;
    }

    VP intersect = ConvexClipPolygon(p1, p2);
    return ComputeSignedArea(intersect) + (intersect.empty() ? 0 : EPS_ABS/2);
}
...
int main() {
    ...
    for (ll cs = 1; cin >> n; ++cs) {
        ...
        while (!feq(hi, lo)) {
            double m1 = (hi+lo)/3;
            double m1_v = area(m1, p1, p2, v1, v2);
            double m2 = 2*(hi+lo)/3;
            double m2_v = area(m2, p1, p2, v1, v2);
